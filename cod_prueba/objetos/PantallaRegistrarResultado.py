...
class PantallaRegistrarResultado:
    ...
    def habilitar(self):
        ...
        if not self.btn_iniciar:
            self.btn_iniciar = tk.Button(self.root, text="Registrar resultado manual", command=self.opcRegistrarResultado)
    ...
    def presentarEventos(self, eventos):
        ...
        if eventos:
            ...
            for idx, evento in enumerate(eventos):
                ...
                btn = tk.Button(frame, text="Seleccionar", command=lambda i=idx: self.seleccionar_evento(eventos, i))
                btn.pack(side=tk.RIGHT, padx=5)
        else:
            messagebox.showinfo("Sin eventos", "No hay eventos sísmicos para revisar.")

    def seleccionar_evento(self, eventos, idx):
